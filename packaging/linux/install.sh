#!/usr/bin/env bash
# Ставит ssh_tunel-linux в /usr/local/bin и готовит службу для текущего
# пользователя. Права администратора нужны только чтобы скопировать файл.
set -euo pipefail

BIN="${1:-./ssh_tunel-linux}"
if [ ! -f "$BIN" ]; then
...
sudo install -m 755 "$BIN" /usr/local/bin/ssh_tunel-linux

echo "Готовлю службу для пользователя $USER..."
mkdir -p ~/.config/systemd/user
install -m 644 "$(dirname "$0")/ssh_tunel.service" ~/.config/systemd/user/ssh_tunel.service
...
Готово. Дальше:
  ...
  3. Адрес веб-интерфейса (с ключом) появится в журнале:
       journalctl --user -u ssh_tunel -n 20
