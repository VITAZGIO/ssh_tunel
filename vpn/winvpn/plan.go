// Пакет winvpn — режим VPN для Windows: виртуальный сетевой адаптер Wintun,
// в который система отдаёт весь трафик, и сетевой стек поверх него, который
// превращает пакеты обратно в соединения и отдаёт их в тот же туннель, что и
// прокси-режим.
//
// В этом файле — решения, не зависящие от Windows: какие адреса и маршруты
// ставить, какой интерфейс считать «настоящим». Они проверяются тестами на
// любой системе; вызовы Windows API — в файлах *_windows.go.
package winvpn
...
const (
	// adapterName — как адаптер называется в «Сетевых подключениях».
