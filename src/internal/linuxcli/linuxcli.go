// Пакет linuxcli — консольная версия для Linux: флаги, журнал в вывод и
// веб-интерфейс. Им пользуются два exe: обычный ssh_tunnel_linux (прокси) и
// ssh_tunnel_vpn_linux (режим VPN, модуль vpn/), — чтобы не разъезжаться.
//
...
// Options отличает версии друг от друга.
type Options struct {
	// Name — имя исполняемого файла для подсказок.
	Name string
	// NetLayer включает режим VPN (см. app.NetLayer). nil — режим прокси.
	NetLayer func(bus *events.Bus) app.NetLayer
}

// Run — вся программа целиком: разбирает флаги и работает до Ctrl+C.
func Run(o Options) {
	if o.Name == "" {
		o.Name = "ssh_tunnel_linux"
	}
	vpn := o.NetLayer != nil
	cfg := config.Load()
	...
	// Без сервера и без веб-интерфейса делать нечего. С веб-интерфейсом —
	// запускаемся: сервер добавляется прямо в нём («Импорт» или вручную).
	// Так установка на домашний сервер — одна и та же для всех, без адреса
	// в командах.
	if p.Host == "" && !*web {
		usage(o.Name)
		os.Exit(1)
	...
	a := app.New(cfg)
	if vpn {
		a.SetNetLayer(o.NetLayer(a.Bus))
	}
	a.RecoverStaleProxy()
	...
	if p.Host == "" {
		fmt.Println("\nСервер ещё не задан — открой веб-интерфейс и добавь его: «Импорт» файла из программы на компьютере или вручную.")
	} else if !*web || cfg.AutoConnectEnabled() {
		fmt.Printf("\nПодключаюсь к %s:%d...\n", p.Host, p.SSHPort)
		...
		fmt.Println("\nАвтоподключение выключено — нажми «Подключить» в веб-панели.")
	}

	if vpn {
		fmt.Print(`
Режим VPN: через туннель идёт весь трафик системы, настраивать прокси не нужно.
...
func usage(name string) {
	fmt.Printf(strings.ReplaceAll(`ssh_tunnel — SSH-туннель до своего сервера

Первый запуск:
  NAME -host ТВОЙ_СЕРВЕР -user root -save

Дальше достаточно:
  NAME              # только туннель, журнал в вывод
  NAME -web         # плюс веб-интерфейс на %s
  NAME -web -web-lan  # панель по адресу машины, без ключа в ссылке
