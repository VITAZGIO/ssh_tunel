...
// Автозапуск на Linux — пользовательская служба systemd (systemctl --user),
// а не системная: программе не нужны права администратора, а настройки и
// ключи лежат в домашней папке. Root требуется ровно в одном месте —
// разрешить службе работать без входа пользователя в систему (loginctl
// enable-linger). Сначала пробуем без пароля: на многих системах это
// разрешено политикой. Не вышло — страница показывает поле для пароля.
package webui
...
func platformBootSupported() bool { return haveSystemd() }

func platformBootEnabled() bool {
	return strings.TrimSpace(output("systemctl", "--user", "is-enabled", unitName)) == "enabled"
}
...
func platformSetBoot(enable bool, password string, flags []string) error {
	if !enable {
