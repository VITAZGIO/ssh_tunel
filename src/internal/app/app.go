...
func (a *App) SwitchProfile(id string) (string, error) {
	...
	// Туннель принадлежал прежнему серверу, поэтому его надо остановить. Но
	// на этом раньше всё и заканчивалось: человек менял сервер и должен был
	// сам нажать «Подключить». Смена сервера при работающем туннеле означает
	// «хочу ходить через тот, другой», поэтому новый поднимаем сами.
