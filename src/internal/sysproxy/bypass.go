...
// winProxyServer — значение для ProxyServer в реестре Windows.
//
// Один адрес на все протоколы, без разбивки вида "http=...;socks=...".
// Разбивка выглядит аккуратнее, но ломает браузеры: схемы, не перечисленные в
// ней явно (в том числе вебсокеты, на которых держатся веб-версии
// мессенджеров), уходят на запись "socks=", а Хром трактует её как SOCKS4 —
// тот работает только с адресами и разрешает имя сам, на этой машине. Запрос
// уходит мимо туннеля и упирается в таймаут.
//
// Отдельная беда той же записи: окно параметров Windows не умеет её показать и
// рисует переключатель выключенным, хотя прокси работает.
//
// Функция вынесена сюда, а не оставлена строкой по месту, чтобы это правило
// можно было закрепить тестом на любой системе.
func winProxyServer(httpAddr string) string { return httpAddr }
