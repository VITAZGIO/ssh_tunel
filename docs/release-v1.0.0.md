...
Пропускает трафик приложений через твой собственный Linux-сервер по обычному
SSH. Один файл, без установки, без прав администратора. Учебный проект.
...
- **Разделение трафика по программам** — через туннель идёт всё, только
  ...
  без переподключения.
- **Локальная сеть остаётся доступной** — роутер, NAS, Home Assistant идут
  напрямую, а не через сервер.
