...
**Если пароль root вообще разрешён** (хостер его выдал и не запрещал вход по
паролю) — всё это можно не делать руками. В программе, на вкладке настроек
сервера, есть кнопка «Настроить сервер по SSH»: адрес, порт, пользователь и
пароль root, и дальше программа сама подключается, кладёт свой ключ, проверяет
вход по нему и по очереди прогоняет ровно те же шаги 2 и 3, что описаны ниже —
с тем же выводом на экран, только построчно и без копирования в терминал.
Пароль root при этом нигде не сохраняется. Если пароль root запрещён (только
ключ от хостера) — этот способ не подходит, тогда весь путь ниже проходится
руками.

## Коротко, весь порядок

Этот документ — ручной путь: пользователь для туннеля заводится и правится
руками, шаг за шагом. Если вместо этого достаточно веб-панели, которая
заводит клиентов сама — по кнопке, без консоли, — она делает то же самое
за один запуск установщика: см. [PANEL_SETUP.md](PANEL_SETUP.md). Шаг 2
(harden.sh) ниже полезен в обоих случаях — это базовая защита самой машины,
она не касается способа заводить клиентов. Шаг 3 (tunnel-user.sh) нужен
только тем, кто **не** ставит панель: панель заводит пользователей для
клиентов сама, тем же способом, что и этот скрипт, только для каждого
клиента отдельно.

1. **Со своего компьютера** — закинуть ключ (шаг 1). Пока это не сделано,
   дальше идти нельзя: скрипт всё равно откажется работать.
2. **На сервере** — вставить блок шага 2, дождаться «Готово».
3. **Вторым окном** — проверить, что вход по ключу работает. Первую сессию не
   закрывать, пока не проверил.
4. **На сервере** — вставить блок шага 3: отдельный пользователь для туннеля.
   Пропусти этот шаг, если дальше будешь ставить панель (шаг 5).
5. **В программе** — адрес сервера, пользователь `tunnel`, свой закрытый ключ.
   Либо вместо шагов 4–5 — [установить панель](PANEL_SETUP.md) и завести
   клиента в ней.

Блоки шагов 2 и 3 можно вставить подряд одним куском — второй скрипт не
зависит от того, что первый показал на экране. Проверку из пункта 3 в этом
случае делай после обоих: она всё так же обязательна.
...
DIRECTIVES="$DIRECTIVES|PermitEmptyPasswords|AllowTcpForwarding|Port|X11Forwarding|MaxStartups"
...
# Сколько соединений может одновременно ждать входа. По умолчанию 10 — а
# ssh_tunnel держит 4 соединения на устройство, и уже три устройства,
# подключающиеся разом, упираются в этот предел. 120 — это 30 устройств;
# число меняется в панели сервера (Настройки → Ёмкость сервера).
...
> Нужен только тем, кто настраивает сервер вручную. Если дальше будет стоять
> веб-панель ([PANEL_SETUP.md](PANEL_SETUP.md)), этот шаг не нужен вовсе — она
> заводит такого пользователя сама на каждого клиента, тем же способом, что
> описан ниже (`restrict,port-forwarding`, своя оболочка `nologin`, без
> дополнительных групп), только не один общий, а отдельный на устройство.
...
# Свои службы ssh_tunnel на этом сервере — тоже исключение: ретранслятор UDP
# (47830) и сеть устройств meshd (47831). Они слушают только 127.0.0.1 и
# доступны лишь через туннель — ради этого и задуманы.
iptables -A SSHTUNNEL -p tcp -d 127.0.0.1 --dport 47830:47831 -j RETURN
