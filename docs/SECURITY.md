...
- Локальную сеть — и это намеренно. Адреса вида `192.168.x.x`, имена без точек
  и суффиксы `.local`/`.lan`/`.home`/`.internal` идут напрямую, иначе домашние
  сервисы (роутер, NAS, Home Assistant) переставали бы открываться при
  включённом туннеле. Переключается настройкой «Локальную сеть тоже вести через
  сервер».
