...
В панели, внизу настроек, есть галочка **«Запускать при старте системы»**.
Программа сама положит файл службы systemd, включит её и разрешит ей работать
без входа пользователя в систему. Последнее требует прав администратора: если
система попросит пароль, прямо там появится поле для него — пароль используется
один раз и никуда не сохраняется.

Рядом, если на машине найден Docker, появляется вторая галочка — включить
